...
######################################             Тестові дані:         #############################################
def testlist():
    frm = Form('Яке місто є столицею України?', 'Київ',
               'Львів', 'Одеса', 'Харків')
    questions_listmodel.form_list.append(frm)

    frm = Form('В якому місті знаходиться Львівський оперний театр?', 'Львів',
               'Київ', 'Одеса',
               'Харків')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яке місто славиться своїми пляжами на Чорному морі?', 'Одеса',
               'Маріуполь', 'Херсон', 'Миколаїв')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яке місто відоме як культурна столиця України?', 'Львів',
               'Дніпро', 'Харків', 'Запоріжжя')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яке місто є важливим промисловим центром і розташоване на Дніпрі?', 'Дніпро',
               'Київ', 'Харків', 'Одеса')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яке місто відоме своїми термальними водами?', 'Трускавець',
               'Свалява', 'Бердянськ', 'Ужгород')
    questions_listmodel.form_list.append(frm)

    frm = Form('В якому місті знаходиться ботанічний сад "Фоміна"?', 'Київ',
               'Львів', 'Харків', 'Одеса')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яке місто є важливим транспортним вузлом на заході України?', 'Львів',
               'Київ', 'Одеса', 'Запоріжжя')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яке місто славиться своїми численними музеями та театрами?', 'Київ',
               'Харків', 'Одеса', 'Черкаси')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яке місто відоме своєю історією та культурною спадщиною,\n зокрема, як "місто на семи пагорбах"?', 'Київ',
               'Львів', 'Чернігів', 'Херсон')
    questions_listmodel.form_list.append(frm)
